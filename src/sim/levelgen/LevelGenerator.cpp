...
void LevelGenerator::generate(int seed)
{
    ...
    for (const auto &p : props)
        if (p.excluder_padding_x != 0.0f || p.excluder_padding_y != 0.0f)
            level_props.emplace_back(Texture::debug, LevelSide::right, p.x - p.excluder_padding_x, p.y - p.excluder_padding_y, p.w + (p.excluder_padding_x
    * 2.0f), p.h + (p.excluder_padding_y * 2.0f));
    */
...
std::vector<LevelFloorInternal> LevelGenerator::generate_impl()
{
    ...
    */
    ...
    const std::array<std::pair<GeneratorFunction, int>, 3> generators = { std::make_pair(grid_generator, 5),
...
void LevelGenerator::generate_segment(std::vector<LevelFloorInternal> &existing_floors,
                                      const int minimum_added,
                                      const LevelSide side,
...
std::vector<LevelFloorInternal> LevelGenerator::generate_grid(const std::vector<LevelFloorInternal> &existing_floors,
                                                              const LevelFloorInternal &start_floor,
                                                              LevelSide start_side)
{
    ...
    for (int i = 0; i < horizontal_tiles; ++i)
    {
        for (int j = 0; j < vertical_tiles; ++j)
        {
            ...
            LevelFloorInternal floor(random_floor(),
                                     start_x,
                                     start_y,
                                     long_horizontal ? floor_width * 2.0f : floor_width,
                                     long_vertical ? floor_height * 2.0f : floor_height);
...
std::vector<LevelFloorInternal> LevelGenerator::generate_linear(const std::vector<LevelFloorInternal> &existing_floors,
                                                                const LevelFloorInternal &start_floor,
                                                                const LevelSide start_side)
{
    std::vector<LevelFloorInternal> generated;
...
std::vector<LevelFloorInternal> LevelGenerator::generate_structure(const std::vector<LevelFloorInternal> &existing_floors,
                                                                   const LevelFloor &start_floor,
                                                                   const LevelSide side,
                                                                   int index)
{
        ...
        for (const auto side : enumerate_connector_sides())
        {
            ...
            if (side == LevelSide::bottom || side == LevelSide::top)
            {
                ...
                for (int i = 0; i < connectors.size(); ++i)
                {
                    ...
                    walls.emplace_back(startx,
                                       y,
                                       i + 1 < connectors.size() ? connectors.at(i + 1).start - startx : ((floor.x + floor.w) - startx),
                                       LevelWallInternal::WIDTH);
            ...
            else
            {
                ...
                {
                    const auto &connector = connectors[i];
                    walls.emplace_back(x,
                                       starty,
                                       LevelWallInternal::WIDTH,
                                       i + 1 < connectors.size() ? connectors.at(i + 1).start - starty : ((floor.y + floor.h) - starty));
                    if (i + 1 < connectors.size())
    ...
    for (int i = 0; i < side_furniture; ++i)
    {
        ...
        for (int j = 0; j < attempts; ++j)
        {
            const LevelPropDefinition &propdef =
                PropDefinitions::get().side_furniture.at(rand.uniform_int(0, PropDefinitions::get().side_furniture.size() - 1));
...
std::vector<LevelFloorConnector> LevelGenerator::get_connectors(const std::vector<LevelFloorConnector> &connectors, LevelSide side)
{
    ...
    std::sort(found.begin(),
              found.end(),
              [](const LevelFloorConnector &a, const LevelFloorConnector &b)
              {
