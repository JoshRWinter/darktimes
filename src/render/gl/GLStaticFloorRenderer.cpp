...
GLStaticFloorRenderer::GLStaticFloorRenderer(win::AssetRoll &roll, const FloorTextureCollection &floor_textures)
	: program(win::load_gl_shaders(roll["shader/static_floor.vert"], roll["shader/static_floor.frag"]))
	, floor_layer_map(floor_textures.get_layer_map())
{
...
std::uint16_t GLStaticFloorRenderer::load(const Renderable &floor)
{
	...
	const int layer_index = floor_layer_map.at(floor.texture);
	...
	if (base_index * 6 > std::numeric_limits<std::uint16_t>::max())
		win::bug("overflow");
