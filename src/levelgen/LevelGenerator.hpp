...
public:
	...
	void generate(int seed, std::vector<LevelFloor> &floors, std::vector<LevelWall> &walls, std::vector<LevelProp> &props);
...
private:
	...
	std::vector<LevelFloorInternal> generate_linear(const std::vector<LevelFloorInternal> &existing_floors, const LevelFloorInternal &start_floor, const LevelSide start_side);
	std::vector<int> find_start_candidates(const std::vector<LevelFloorInternal> &floors);
	static std::vector<int> find_neighbors(const std::vector<LevelFloorInternal> &floors, const LevelFloorInternal& floor/*, LevelSide side*/);
	...
	static std::vector<PropExcluder> generate_door_excluders(const LevelFloorInternal &floor);
	std::vector<LevelPropInternal> generate_new_props(const LevelFloorInternal &floor, const std::vector<PropExcluder> &excluders);
	//std::vector<LevelPropInternal> generate_props_from_spawns(const LevelFloorInternal &floor, const std::vector<LevelPropInternal> &excluders);
